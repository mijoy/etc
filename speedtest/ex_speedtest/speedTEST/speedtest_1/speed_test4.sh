...
for i in {1..10}
do
    /usr/bin/time -a -o $time_output -f  "${i} \t%e" $command 
done
manage_file
}
awk_ok_sleep ()
...
function mainopt
{
	...
	echo "  [2] File Merge                        "
	echo "  [3] Gnuplot(Linear)                   "
	echo "  [4] Gnuplot(Histogram)                "
	echo "----------------------------------------"
	echo "----------------------------------------"
	echo "  [5] Open fit log                      "
	echo "  [6] Gnuplot Edit(linear)              "
	echo "  [7] Gnuplot Edit(histogram)           "
	echo "----------------------------------------"
	echo "----------------------------------------"
	echo "  [q] Exit                              "  
	echo "  [0] Help                              "
	...
	case $opt in
	    ...
	    2) file_merge;;
	    3) ./gnuplot_linear.sh;;
	    4) ./gnuplot_histogram.sh;;
	    5) gedit fit.log&;;
	    6) emacs gnuplot_linear.sh&;;
	    7) emacs gnuplot_histogram.sh&;;
	    q) exit;;
	    0) help;;
	    *) echo "Invalid option, choose again...";help; 
		echo "Press Enter to continue... "; read;;
